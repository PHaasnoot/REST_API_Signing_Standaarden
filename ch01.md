...
Op moment van schrijven is dit de laatste versie van de standaard: _draft-ietf-httpbis-message-signatures-17_ [[HTTP-MessageSig]].
...
_Note that the RSA PSS algorithm in use here is non-deterministic, meaning a different signature value will be created every time the algorithm is run. The signature value provided here can be validated against the given keys, but newly-generated signature values are not expected to match the example._ c
